...
4.  **Context Window**: In Normal mode, this is the configured local context/KV capacity. In Agentic mode, the same slider is labeled **Active Working Context** and represents the agent's bounded working context. Both modes use the slider value directly in the same architecture-aware KV-cache formula.
