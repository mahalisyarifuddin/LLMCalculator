...
4.  **Workload Profiles**: Choose **Chatbot**, **Embedded / local app**, **Long-horizon agent**, or **Hosted API planner**. Every profile budgets retained input plus reserved completion, peak resident sequences, and KV allocation headroom. Embedded mode also supports a separate runtime/activation reserve.
5.  **Agentic Workload Planning**: The agent profile adds full-history, rolling-history, or structured-summary-and-retrieval planning plus a total task horizon. Horizon estimates workflow continuity and does not allocate one giant KV cache.
...
-   **Workload-Aware Context Planner**: Covers chatbot, embedded/local, long-horizon agent, and hosted-API planning. Separates VRAM-resident context from task horizon; budgets output reserve, peak resident sequences, KV allocation headroom, runtime reserve, and configurable context/output caps.
