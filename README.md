...
4.  **Agentic Workload Planning**: Optional long-horizon controls reserve completion tokens, multiply KV cache by concurrent sequences, and add KV safety headroom. A total task horizon estimates rolling working windows but does not allocate one giant KV cache.
5.  **KV Cache**: Memory required to store Key-Value states for the active context. It also supports separate quantization for KV cache.
6.  **System Overhead:**
...
-   **Agentic Workload Planner**: Separates VRAM-resident active KV context from total task horizon; budget completion reserve, concurrent sequences, and KV safety headroom for long-running agent harnesses.
